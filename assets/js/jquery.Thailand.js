...
$.Thailand = function (options) {
  ...
  let preprocess = function (data) {
    ...
    loadDB = function (callback) {
      let type = options.database_type.toLowerCase(),
        xhr;
          ...
          xhr.onreadystatechange = function () {
            if (xhr.readyState === 4) {
              if (xhr.status === 200) {
                    ...
                    zipReader.getEntries(function (r) {
                      r[0].getData(new zip.BlobWriter(), function (blob) {
                        ...
                        reader.onload = function () {
                          callback(
                            new JQL(preprocess(JSON.parse(reader.result)))
                          );
                        };
                ...
                throw new Error(
                  'File "' + options.database + '" is not exists.'
                );
              }
            }
          };
  ...
  loadDB(function (DB) {
    ...
    let i,
      key,
      defaultTemplates = {
        // template of autocomplete choices
        empty: " ",
      ...
      autocomplete_handler = function (e, v) {
        // set value when user selected autocomplete choice
    ...
    let templates =
      typeof options.templates === "object"
        ? Object.assign(defaultTemplates, options.templates)
        : defaultTemplates;
        ...
        options[i]
          .typeahead(
            ...
            },
            {
              ...
              templates: templates,
              source: function (str, callback, asyncCallback) {
                const wsurl = `${MAIN_URL}/api/get-geo-data`;
                $.ajax({
                  url: wsurl,
                  method: "POST",
                  contentType: "application/json",
                  data: JSON.stringify({ text: str, orderBy: field }),
                  dataType: "json",
                  success: function (res) {
                    let resultData = res.resultData;
                    let possibles = [];
                    possibles = $.map(resultData, (el) => ({
                      district_code: el.sub_district_id,
                      amphoe: el.district_name,
                      province: el.province_name,
                      zipcode: el.zipcode,
                      district: el.sub_district_name,
                      amphoe_code: el.district_id,
                      province_code: el.province_id,
                    }));
                    asyncCallback(possibles);
                  },
