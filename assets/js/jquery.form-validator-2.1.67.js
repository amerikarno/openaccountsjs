        ...
        _setInlineErrorMessage = function($input, mess, conf, $messageContainer) {
            ...
            if( custom ) {
                ...
                if( $mess.length == 0 ) {
                    ...
                    if($input.attr('type') != 'text' && $input.attr('type') != 'password' && $input.attr('type') != 'radio'){
                        $('#error'+$input.attr('name')).html('');
                        $mess.appendTo('#error'+$input.attr('name'));   
                    }else{
        ...
        if ( !$.formUtils.haltValidation && errorInputs.length > 0 ) {
            ...
            if( displayError ) {
                ...
                if(conf.scrollToTopOnError) {
                    $window.scrollTop($form.find('.error:first').offset().top - 60);
                }
            }
    ...
    $.formUtils = {
        ...
        validateInput : function($elem, language, conf, $form, eventContext) {
            ...
            $.split(validationRules, function(rule) {
                ...
                if( validator && typeof validator['validatorFunction'] == 'function' ) {
                    ...
                    if( eventContext != 'keyup' || validator.validateOnKeyUp ) {
                        isValid = validator.validatorFunction(value, $elem, conf, language, $form);
                        var datanoti = $elem.attr("datanoti");
                    }
                    if(datanoti == "not_20" || datanoti == "day_wrong") {
                        validationErrorMsg =  $elem.attr(conf.validationErrorMsgAttribute+'-'+rule.replace('validate_', ''));
                        if( !validationErrorMsg ) {
                            validationErrorMsg =  $elem.attr(conf.validationErrorMsgAttribute);
                            if( !validationErrorMsg ) {
        ...
        */
        LANG : {
            ...
            requiredFields : 'Please select one choice.',
            ...
            groupCheckedTooFewStart : 'กรุณาเลือกประเภทบ้านที่ต้องการ',
            groupCheckedTooManyStart : 'Please choose a maximum of ',
            groupCheckedEnd : ' ',
            badCreditCard : 'The credit card number is not correct',
            badCVV : 'The CVV number was not correct',
            age_less_18 : 'ท่านไม่สามารถสมัครขอเปิดบัญชีได้ เนื่องจากอายุไม่ถึงเกณฑ์ 18 ปีบริบูรณ์',
            idNumber : 'ตัวเลข ',
            notCorrect : ' หลักไม่ถูกต้อง',
            idNull : ' กรุณาระบุ หมายเลขบัตรประชาชน ',
            idNullEnd : ' หลัก',
            pleaseDay : ' กรุณาระบุวันที่',
            laserId : 'ตัวอักษรและตัวเลขรวม 12 หลัก'
        }
    };
            ...
            switch ( $el.attr('type') ) {
                case 'checkbox':
                    if($el.is(':checked') == false)
                        $("#error"+$el.attr('name')).show()
                    else
                        $("#error"+$el.attr('name')).hide()
    ...
    */
    ...
    validatorFunction : function(val, $el, conf, lang) {
        ...
        {   // outside of allowed range
            ...
            case "min":
                // this.errorMessage = lang.lengthTooShortStart + lengthCheckResults[1] + lang.lengthBadEnd;
                this.errorMessage = lang.pleaseDay;
                checkResult = false;
                break;
            // too long
            case "max":
    ...
    validatorFunction : function(val, $el, conf, lang) {
        ...
        {   // outside of allowed range
            ...
            // too short
            case "min":
                this.errorMessage = lang.laserId;
                checkResult = false;
                break;
            // too long
            case "max":
    ...
    $.formUtils.addValidator({
        ...
        validatorFunction : function(val, $el, conf, lang) {
            if(val !== '') {               
                if((val.substr(0,2) == '24' || val.substr(0,2) == '25') 
                    && val.length > 3
                    && val.replace(/[^A-Za-z\+\-\=\~\!@#\%&\*\(\)_\+\\\/<>\?\{\}\.\$‘\^\+\"\';:,\s]+/, '') === '' 
                    && val.replace(/[^ก-๙]+/, '') === '' ) { 
                    return true;
    ...
    $.formUtils.addValidator({
        ...
        validatorFunction : function(val, $el, conf, lang) {
            ...
            {   // outside of allowed range
                ...
                // too short
                case "min":
                    this.errorMessage = lang.idNull + lengthCheckResults[1] + lang.idNullEnd;
                    checkResult = false;
                    break;
                // too long
                case "max":
            ...
            {   // outside of allowed range
                ...
                // too short
                case "min":
                    this.errorMessage = lang.idNumber + lengthCheckResults[1] + lang.notCorrect;
                    checkResult = false;
                    break;
                // too long
                case "max":
    ...
    $.formUtils.addValidator({
        ...
        validatorFunction: function (val, $el, conf, lang) {
            ...
            if (val.length < 10) {
                ...
                var i = 0;
                for (i = 0, sum = 0; i < 9; i++) {
                    sum += parseFloat(val.charAt(i)) * (10 - i);
                }
                if ((8 - sum % 8) % 7 != parseFloat(val.charAt(9))) {
                    return false;
                }
            }
            // check if length is above min, below max or within range.
